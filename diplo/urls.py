...
urlpatterns = [
    ...
    url(r'^tema/(?P<tema>\d+)/(?P<pk>\d+)/dados/$', data_views.IndicadorDetail.as_view(), name='indicador-detail'),
    url(r'^tema/(?P<tema>\d+)/(?P<pk>\d+)/(?P<regionalizacao>\w+)/dados/$', data_views.IndicadorDetail.as_view(), name='indicador-detail'),
